...
The first job of a project starts the builder (about 30 s) and waits for it, so even the first
build fills the cache. A builder that nobody used for 24 hours is deleted together with its cache.
